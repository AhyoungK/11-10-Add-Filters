    ...
    try:
        ...
        if hands:
            ...
            if fingers1[0] == 0:
                ...
                cameraFeedImg = cv2.divde(grayscaleImage, 255 - blurredImg, scale=256)
            elif fingers1[3] == 1:
            ...
            if fingers2[0] == 0:
                currentFingerUp = "Thumb"
                cameraFeedImg = cv2.ctvColor(cameraFeedImg, cv2.COLOR_BGR2GRAY)
            elif fingers2[1] == 1:
                currentFingerUp = "Index Finger"
                cameraFeedImg = cv2.xphoto.oilPainting(cameraFeedImg, size = 7, dynRatio=1)
                ...
                currentFingerUp = "Middle Finger"
                grayscaleImg = cv2.cvtColor(cameraFeedImg, cv2.COLOR_BGR2GRAY)
                intertedImg = 255 - grayscaleImage
                blurredImg = cv2.GaussianBlur(invertedImg, (21, 21), 0)
